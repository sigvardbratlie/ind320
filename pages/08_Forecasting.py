...
df_w = get_weather_data(st.session_state.get("location",{}).get("coordinates"),dates = st.session_state.dates, set_time_index=True,)
...
y = st.selectbox("Select target variable for forecasting", options=df_m.columns, index=2)


start_idx = df_m.index.get_loc(trainin_time[0])
end_idx = df_m.index.get_loc(trainin_time[1])
y_data = df_m[y]
x_data = df_m.drop(columns=[y])
#st.dataframe(x_data)
...
t_f = forecast.values == y_data.iloc[end_idx:].values
st.write(f"Forecast matches actual data: {t_f.all()}")
...
#st.dataframe(y_data.head())

# Plot
fig = go.Figure()
fig.add_trace(go.Scatter(x=y_data.index[:end_idx], y=y_data.iloc[:end_idx], name='Training'))
fig.add_trace(go.Scatter(x=y_data.index[end_idx:], y=y_data.iloc[end_idx:], name='Actual'))
